        ...
        try (CloseableIterable<Bundle> bundleIter = Bundle.bundleStream(inputStream)) {
            for (Bundle rawBundle : bundleIter) {
                try {
                    ...
                    final Accessor accessor = actioner.as(Accessor.class);
                    final ContentTypes contentType = aclManager.getContentType(bundle.getType());
                    permissionUtils.checkContentPermission(accessor, contentType, PermissionType.CREATE);
                    permissionUtils.checkContentPermission(accessor, contentType, PermissionType.UPDATE);
