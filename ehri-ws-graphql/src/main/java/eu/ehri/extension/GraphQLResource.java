...
public class GraphQLResource extends AbstractAccessibleResource<Accessible> {
    ...
    private StreamingOutput lazyExecution(GraphQLSchema schema, GraphQLQuery q) {
        ...
        return outputStream -> {
            try (final Tx tx = beginTx(); JsonGenerator generator = jsonFactory.createGenerator(outputStream).useDefaultPrettyPrinter()) {
                StreamingGraphQL ql2 = new StreamingGraphQL(new GraphQLImpl(api(), true).getSchema());
